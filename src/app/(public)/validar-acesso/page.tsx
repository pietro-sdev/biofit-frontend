  ...
  export default function ValidarAcesso() {
    ...
    const handleValidation = async () => {
      if (!token.trim()) {
        toast({ title: "Erro", description: "O token é obrigatório.", variant: "destructive" });
        return;
      }

      setLoading(true);
      try {
        const response = await fetch(`${process.env.NEXT_PUBLIC_API_URL}/token/validate/${token}`);
        ...
        toast({ title: "Sucesso!", description: "Token validado com sucesso!", variant: "success" });
      } catch (error) {
        setIsValid(false);
        setGroupLink(null);
      ...
      <div className="flex justify-center items-center min-h-screen bg-gray-100">
        <Card className="w-full max-w-md shadow-lg">
          ...
          <CardContent className="space-y-4">
            ...
            {isValid === false && <p className="text-red-500 text-center">Token inválido. Tente novamente.</p>}
              
            {/* ⚠️ Se o token for válido e tivermos um groupLink, mostramos o botão */}
            {isValid && groupLink && (
